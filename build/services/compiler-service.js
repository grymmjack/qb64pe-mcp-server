...
class QB64PECompilerService {
    ...
    formatOpaqueCompileFailure(execError, command, sourceFilePath, resolvedOutputPath) {
        const details = [
            execError?.message ||
    ...
    async tryCaptureCompileFailureWithPty(command, cwd) {
        ...
        try {
            ...
            const combinedOutput = `${ptyError?.stdout || ""}${ptyError?.stderr || ""}`.trim();
            return combinedOutput.length > 0 ? combinedOutput : null;
        }
    }
    getVSCodeConfiguredCompilerPath(sourceFilePath) {
        const fs = require("fs");
        const os = require("os");
        const path = require("path");
        const settingsFiles = [];
        ...
        if (process.platform === "win32") {
            ...
            settingsFiles.push(path.join(homeDir, "Library", "Application Support", "Code", "User", "settings.json"));
        }
        else {
            settingsFiles.push(path.join(homeDir, ".config", "Code", "User", "settings.json"));
        }
    ...
    compilerOptions = [
        {
            flag: "-x",
            description: "REQUIRED for headless/CLI use. Compile-only mode — does NOT launch the QB64PE IDE GUI. " +
        ...
        {
            flag: "-w",
            description: "REQUIRED for headless/CLI use. Suppresses interactive warning dialogs that would otherwise block " +
        ...
        {
            flag: "-c",
            description: "Compile source file and show progress in QB64PE's own window. DO NOT use in headless/terminal environments — use -x instead.",
            platform: ["windows", "macos", "linux"],
        ...
        {
            flag: "-q",
            description: "Quiet mode — suppress compiler banner output. Often combined with -m and -x for clean headless output.",
            platform: ["windows", "macos", "linux"],
        ...
        {
            flag: "-m",
            description: "Message mode — send compiler progress and error messages to stdout/stderr. " +
        ...
        },
        {
            ...
            description: "Set the maximum number of parallel C++ compilation processes. Speeds up compilation significantly on multi-core systems. " +
            ...
            platform: ["windows", "macos", "linux"],
            example: "qb64pe -w -x -f:MaxCompilerProcesses=12 myprogram.bas -o myprogram.run",
...
PRINT "Final count: "; count`,
     ...
     */
    async getCompilerOptions(platform = "all", optionType = "all") {
        ...
        return {
            headlessWorkflow: {
                summary: "QB64PE HEADLESS / CLI COMPILATION — Always use these flags when compiling from a terminal or LLM agent workflow. " +
                ...
                recommendedCommand: "qb64pe -w -x -f:MaxCompilerProcesses=<N> SOURCE.BAS -o OUTPUT.run",
                flags: {
                    ...
                    "-o PATH": "Output binary path. Omit the extension on Linux/macOS (or use .exe on Windows).",
                    "-f:MaxCompilerProcesses=N": "Parallel C++ compilation threads (set N = CPU core count for fastest builds, e.g. 12).",
                ...
                warning: "Running `qb64pe SOURCE.BAS` without -x WILL attempt to open the IDE GUI window and hang in any headless/terminal environment.",
            },
            options,
        };
    }
     ...
     */
    async getDebuggingHelp(issue, platform = "all") {
     ...
     */
    async getCompilerReference() {
        let reference = "# QB64PE Compiler Reference\n\n";
        reference += "## Compilation Options\n\n";
        const compilationOptions = (await this.getCompilerOptions("all", "compilation")).options;
        ...
        const optimizationOptions = (await this.getCompilerOptions("all", "optimization")).options;
     ...
     */
    findRelevantDebuggingTechniques(issue, platform) {
     ...
     */
    getGeneralDebuggingAdvice(platform) {
...
`;
     ...
     */
    getAdditionalDebuggingTips(platform) {
     ...
     */
    async tryVSCodeTask(sourceFilePath) {
    ...
    async compileAndVerify(sourceFilePath, qb64pePath, compilerFlags, useStoredFlags = true) {
        ...
        if (compilerFlags) {
            ...
            if (normalized.removedFlags.length > 0) {
                result.suggestions.push(`ℹ️ Ignored incompatible verification flags: ${JSON.stringify(normalized.removedFlags)}`);
            }
        }
            ...
            if (normalized.removedFlags.length > 0) {
                result.suggestions.push(`ℹ️ Ignored stored incompatible verification flags: ${JSON.stringify(normalized.removedFlags)}`);
            }
        }
        else {
        ...
        try {
            ...
            if (!qb64peExe) {
                const configuredCompilerPath = this.getVSCodeConfiguredCompilerPath(sourceFilePath);
                if (configuredCompilerPath) {
                    qb64peExe = configuredCompilerPath;
                    result.suggestions.push(`ℹ️ Using QB64PE compilerPath from VS Code settings: ${configuredCompilerPath}`);
                }
            }
            if (!qb64peExe) {
            ...
            const cmd = `"${qb64peExe}" ${flags.join(" ")} -o "${resolvedOutputPath}" "${sourceFilePath}"`;
            // Execute compilation
            try {
                const { stdout, stderr } = await execAsync(cmd, {
                    cwd: outputDir,
                ...
                const executablePath = this.resolveExecutablePath(sourceFilePath, outputName, resolvedOutputPath);
                if (fs.existsSync(executablePath)) {
                    result.success = true;
                    result.executablePath = executablePath;
                ...
                const directOutput = `${execError?.stdout || ""}${execError?.stderr || ""}`.trim();
                if (directOutput.length > 0) {
                    result.output = directOutput;
                }
                else {
                    const ptyOutput = await this.tryCaptureCompileFailureWithPty(cmd, outputDir);
                    if (ptyOutput) {
                        result.output = ptyOutput;
                        result.suggestions.push("ℹ️ QB64PE produced no normal stdout/stderr; captured compiler output through a pseudo-terminal fallback.");
                    }
                    else {
                        result.output = this.formatOpaqueCompileFailure(execError, cmd, sourceFilePath, resolvedOutputPath);
                        result.suggestions.push("⚠️ QB64PE returned no compiler output. Included process metadata instead of a generic shell failure message.");
                    }
                }
                this.parseCompilationOutput(result.output, result.errors, result.suggestions);
    ...
    parseCompilationOutput(output, errors, suggestions) {
        ...
        for (const line of lines) {
            ...
            const errorMatch = trimmed.match(/(?:error|line)\s+(?:on\s+)?(?:line\s+)?(\d+)[:\s]+(.+)/i);
            if (errorMatch) {
                errors.push({
            ...
            const warningMatch = trimmed.match(/warning[:\s]+(?:line\s+)?(\d+)?[:\s]*(.+)/i);
            if (warningMatch) {
                errors.push({
...
exports.QB64PECompilerService = QB64PECompilerService;
