...
exports.QB64PEExecutionService = void 0;
...
let graphicsKeywordsCache = null;
/**
...
class QB64PEExecutionService {
     ...
     */
    analyzeExecutionMode(sourceCode) {
     ...
     */
    detectGraphicsUsage(sourceCode) {
     ...
     */
    getExecutionGuidance(executionState) {
        const guidance = {
            recommendation: '',
     ...
     */
    getProcessMonitoringCommands(processName = 'qb64pe') {
     ...
     */
    getProcessTerminationCommands(pid) {
        switch (this.platform) {
            case 'win32':
                return [
     ...
     */
    generateMonitoringTemplate(originalCode) {
...
`;
    ...
    parseConsoleOutput(output) {
        ...
        const isWaitingForInput = inputPrompts.some(prompt => lastFewLines.includes(prompt));
        // Check for completion signals
        const completionSignals = [
            'end', 'finished', 'complete', 'done', 'exit', 'system',
            'program completed', 'execution finished', 'closing'
        ];
        const isCompleted = completionSignals.some(signal => lastFewLines.includes(signal));
     ...
     */
    getFileMonitoringCommands(logFile) {
...
exports.QB64PEExecutionService = QB64PEExecutionService;
