...
exports.QB64PEPortingService = void 0;
class QB64PEPortingService {
    transformations = [];
    warnings = [];
    errors = [];
    /**
     ...
     */
    async portQBasicToQB64PE(sourceCode, options = {}) {
        const opts = {
     ...
     */
    addQB64PEMetacommands(code, options) {
        if (!options.addModernFeatures)
            return code;
     ...
     */
    removeForwardDeclarations(code) {
        const lines = code.split('\n');
        const removedLines = [];
    ...
    convertDefFnToFunctions(code) {
        ...
        for (const match of multilineMatches) {
            ...
            const processedParams = parameters.split(',').map((p) => {
                const param = p.trim();
...
End Function`;
     ...
     */
    convertMathematicalConstants(code) {
        let convertedCode = code;
     ...
     */
    addGraphicsEnhancements(code, options) {
        if (!options.convertGraphics)
            return code;
        let convertedCode = code;
        const hasGraphics = this.detectGraphics(code);
        if (hasGraphics) {
     ...
     */
    optimizeCompatibility(code, options) {
        let convertedCode = code;
     ...
     */
    removeNoPrefixMetacommand(code) {
        const lines = code.split('\n');
        const filteredLines = [];
        let removed = false;
...
exports.QB64PEPortingService = QB64PEPortingService;
//# sourceMappingURL=porting-service.js.map