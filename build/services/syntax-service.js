...
class QB64PESyntaxService {
     ...
     */
    checkVariableNamingMismatch(lines, compatibilityIssues) {
        // Track variables declared with DIM AS (without sigil)
        const declaredVars = new Map();
        // Type sigils mapping
        const typeSigils = new Map([
            ["INTEGER", "%"],
            ["LONG", "&"],
            ["SINGLE", "!"],
            ["DOUBLE", "#"],
        ...
        lines.forEach((line, index) => {
            ...
            const dimMatch = trimmedLine.match(/^DIM\s+(?:SHARED\s+)?([A-Z_][A-Z0-9_]*)\s+AS\s+(INTEGER|LONG|SINGLE|DOUBLE|STRING|_BYTE|_INTEGER64|_UNSIGNED\s+(?:INTEGER|LONG))/i);
            if (dimMatch) {
        ...
        lines.forEach((line, index) => {
            ...
            while ((match = varPattern.exec(line)) !== null) {
                ...
                const sigilMatch = varWithSigil.match(/^([A-Z_][A-Z0-9_]*)([%&!#$]{1,2})$/);
                if (!sigilMatch)
                    continue;
            ...
            tokens.forEach((token, columnIndex) => {
                ...
                if (cleanToken.length > 1 && /^[A-Z_][A-Z0-9_]*$/.test(cleanToken)) {
                    ...
                    if (!validation.isValid &&
                        ...
                        if (validation.keyword.version === "QB64PE" &&
    ...
    calculateSyntaxScore(lines, errors, warnings) {
        ...
        const commentLines = lines.filter((line) => line.trim().startsWith("'") || line.trim().startsWith("REM")).length;
