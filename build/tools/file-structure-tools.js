...
exports.registerFileStructureTools = registerFileStructureTools;
const zod_1 = require("zod");
/**
 * Register file structure validation tools
 */
function registerFileStructureTools(server, services) {
    // Tool 1: Validate .BI file structure
    server.registerTool("validate_bi_file_structure", {
        ...
        inputSchema: {
            content: zod_1.z.string().describe("Content of the .BI file to validate"),
            filename: zod_1.z.string().optional().describe("Filename for context (optional)"),
        },
    }, async ({ content, filename }) => {
        const result = services.fileStructureService.validateBIFile(content);
        // Format output
    ...
    server.registerTool("validate_bm_file_structure", {
        ...
        inputSchema: {
            content: zod_1.z.string().describe("Content of the .BM file to validate"),
            filename: zod_1.z.string().optional().describe("Filename for context (optional)"),
        },
    }, async ({ content, filename }) => {
        const result = services.fileStructureService.validateBMFile(content);
    ...
    server.registerTool("validate_qb64_gj_lib_file_pair", {
        ...
        inputSchema: {
            biContent: zod_1.z.string().describe("Content of the .BI file"),
            bmContent: zod_1.z.string().describe("Content of the .BM file"),
            libraryName: zod_1.z.string().optional().describe("Name of the library (for context)"),
        },
    }, async ({ biContent, bmContent, libraryName }) => {
        const result = services.fileStructureService.followsGJLibConventions(biContent, bmContent);
    ...
    server.registerTool("quick_check_qb64_file_structure", {
        ...
        inputSchema: {
            filename: zod_1.z.string().describe("Filename (with .BI or .BM extension)"),
            content: zod_1.z.string().describe("File content to validate"),
        },
    }, async ({ filename, content }) => {
        const result = services.fileStructureService.validateFile(filename, content);
...
//# sourceMappingURL=file-structure-tools.js.map