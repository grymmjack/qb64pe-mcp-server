...
exports.registerInstallationTools = registerInstallationTools;
const zod_1 = require("zod");
const mcp_helpers_js_1 = require("../utils/mcp-helpers.js");
/**
 * Register all installation-related tools
 */
function registerInstallationTools(server, services) {
    server.registerTool("detect_qb64pe_installation", {
        title: "Detect QB64PE Installation",
        description: "Automatically detect QB64PE installation on the system",
        inputSchema: {},
    }, async () => {
        try {
            const installation = await services.installationService.detectInstallation();
            return (0, mcp_helpers_js_1.createMCPResponse)(installation);
        }
        catch (error) {
            return (0, mcp_helpers_js_1.createMCPError)(error, "detecting QB64PE installation");
        }
    });
    server.registerTool("get_qb64pe_path_configuration", {
        ...
        try {
            const config = services.installationService.getPathConfiguration(installPath);
            ...
            return (0, mcp_helpers_js_1.createMCPError)(error, "getting PATH configuration");
        }
    });
    server.registerTool("validate_qb64pe_path", {
        ...
        try {
            const validation = await services.installationService.validatePath(testPath);
            return (0, mcp_helpers_js_1.createMCPResponse)({
                testPath,
                validation,
            ...
            return (0, mcp_helpers_js_1.createMCPError)(error, "validating path");
        }
    });
    server.registerTool("generate_qb64pe_installation_report", {
        ...
        try {
            const report = await services.installationService.generateInstallationReport();
            return (0, mcp_helpers_js_1.createMCPTextResponse)(report);
        }
        catch (error) {
            return (0, mcp_helpers_js_1.createMCPError)(error, "generating installation report");
        }
    });
    server.registerTool("get_qb64pe_installation_guidance", {
        ...
        try {
            const installation = await services.installationService.detectInstallation();
            const guidance = services.installationService.generateInstallationGuidance(installation);
...
Remember: QB64PE installation and PATH configuration is often the first hurdle for users!`;
            return (0, mcp_helpers_js_1.createMCPTextResponse)(llmGuidance);
        }
        catch (error) {
            return (0, mcp_helpers_js_1.createMCPError)(error, "getting installation guidance");
        }
    });
}
