...
exports.registerMouseInputTools = registerMouseInputTools;
const zod_1 = require("zod");
const mcp_helpers_js_1 = require("../utils/mcp-helpers.js");
/**
 * Register all mouse input-related tools
 */
function registerMouseInputTools(server, services) {
    // Tool 1: Validate _MOUSEWHEEL usage
    server.registerTool("validate_mouse_wheel_usage", {
        title: "Validate _MOUSEWHEEL Usage",
        description: "🐭 WHEN TO USE: When implementing mouse wheel features (zoom, scroll) or debugging why wheel input isn't working.\n\n" +
        ...
        try {
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                if (line.includes("WHILE _MOUSEINPUT") ||
                ...
                if (line.includes("_MOUSEWHEEL")) {
                    if (!insideMouseInputLoop) {
                        issues.push({
                            line: lineNum,
                            code: lines[i].trim(),
                            issue: "_MOUSEWHEEL called OUTSIDE _MOUSEINPUT loop - will always return 0",
                            severity: "critical",
                            fix: "Move _MOUSEWHEEL accumulation INSIDE the _MOUSEINPUT loop:\n" +
                                "wheel_delta% = 0\n" +
                    ...
                    if (line.includes("DO WHILE") ||
                        ...
                        if (!line.includes("_MOUSEINPUT")) {
                            issues.push({
                                line: lineNum,
                                code: lines[i].trim(),
                                issue: "Nested loop inside _MOUSEINPUT may cause performance issues",
                                severity: "warning",
            ...
            return (0, mcp_helpers_js_1.createMCPError)(error, "validating mouse wheel usage");
        }
    });
    // Tool 2: Get mouse input best practices
    server.registerTool("get_mouse_input_best_practices", {
        title: "Get Mouse Input Best Practices",
        description: "🐭 WHEN TO USE: When implementing mouse handling or experiencing input lag/performance issues.\n\n" +
        ...
        try {
            const patterns = {
                ...
                "per-event": {
                    ...
                    template: includeExamples
                        ...
                        : undefined,
                    caution: "⚠️ Do NOT put expensive operations inside this loop - they will run 10-100x per frame!",
                    antiPattern: [
            ...
            const response = {
                patterns: pattern === "all" ? patterns : { [pattern]: patterns[pattern] },
                criticalRules: [
                ...
                performanceGuideline: {
                    ...
                    explanation: "The _MOUSEINPUT loop drains the event buffer (10-100 events per frame). Processing must happen OUTSIDE this loop.",
                },
            };
            if (includeExamples) {
            ...
            return (0, mcp_helpers_js_1.createMCPResponse)(response);
        }
        catch (error) {
            return (0, mcp_helpers_js_1.createMCPError)(error, "getting mouse input best practices");
        }
    });
    // Tool 3: Generate state machine template
    server.registerTool("generate_state_machine_template", {
        title: "Generate State Machine Template",
        description: "🎰 WHEN TO USE: When creating complex interactive tools with multiple modes (import, marquee, move, etc.).\n\n" +
        ...
        inputSchema: {
            ...
            stateNames: zod_1.z
                ...
                .describe("Array of state names (e.g., ['IDLE', 'LOADED', 'PLACING', 'PANNING'])"),
            includeKeyboardHandler: zod_1.z
                .boolean()
                .optional()
                .describe("Include keyboard input handler template (default: true)"),
            includeMouseHandler: zod_1.z
                .boolean()
                .optional()
                .describe("Include mouse input handler template (default: true)"),
        },
    }, async ({ toolName, stateNames, includeKeyboardHandler = true, includeMouseHandler = true, }) => {
        try {
            ...
            if (includeMouseHandler) {
                ...
                stateNames.forEach((state) => {
                    ...
                    const functionState = state
                        .toLowerCase()
                        .replace(/[^a-z0-9]/g, "_");
                    mouseHandler.push(`        CASE ${upperToolName}_STATE_${upperState}`, `            ${upperToolName}_HANDLE_MOUSE_${upperState}(tool)`);
                });
                mouseHandler.push(`    END SELECT`, `END SUB`, ``);
                // Generate state-specific mouse handler stubs
                stateNames.forEach((state) => {
                    const upperState = state.toUpperCase().replace(/[^A-Z0-9]/g, "_");
                    mouseHandler.push(`SUB ${upperToolName}_HANDLE_MOUSE_${upperState} (tool AS ${upperToolName}_TYPE)`, `    ' TODO: Implement ${state} state mouse handling`, `    `, `    ' Example patterns:`, `    ' - Click detection: IF tool.MOUSE_BUTTON THEN ...`, `    ' - Wheel zoom: IF tool.WHEEL_DELTA <> 0 THEN zoom = zoom + tool.WHEEL_DELTA * 0.1`, `    ' - Drag: IF tool.MOUSE_BUTTON THEN offset_x = tool.MOUSE_X - start_x`, `    ' - State transitions: ${upperToolName}_SET_STATE tool, ${upperToolName}_STATE_NEXT`, `END SUB`, ``);
                });
            ...
            if (includeKeyboardHandler) {
                ...
                stateNames.forEach((state) => {
                    const upperState = state.toUpperCase().replace(/[^A-Z0-9]/g, "_");
                    keyboardHandler.push(`        CASE ${upperToolName}_STATE_${upperState}`, `            ${upperToolName}_HANDLE_KEYBOARD_${upperState}(tool, k)`);
                });
                keyboardHandler.push(`    END SELECT`, `END SUB`, ``);
                // Generate state-specific keyboard handler stubs
                stateNames.forEach((state) => {
                    const upperState = state.toUpperCase().replace(/[^A-Z0-9]/g, "_");
                    keyboardHandler.push(`SUB ${upperToolName}_HANDLE_KEYBOARD_${upperState} (tool AS ${upperToolName}_TYPE, k AS LONG)`, `    ' TODO: Implement ${state} state keyboard handling`, `    `, `    ' Example patterns:`, `    ' SELECT CASE k`, `    '     CASE 27 ' ESC`, `    '         ${upperToolName}_SET_STATE tool, ${upperToolName}_STATE_${stateNames[0].toUpperCase().replace(/[^A-Z0-9]/g, "_")}`, `    '     CASE 13 ' ENTER`, `    '         ${upperToolName}_SET_STATE tool, ${upperToolName}_STATE_NEXT`, `    ' END SELECT`, `END SUB`, ``);
                });
            ...
            stateNames.forEach((state) => {
                const upperState = state.toUpperCase().replace(/[^A-Z0-9]/g, "_");
                usageExample.push(`        CASE ${upperToolName}_STATE_${upperState}`, `            ' Render ${state} state`);
            });
            usageExample.push(`    END SELECT`, `    `, `    _DISPLAY`, `    _LIMIT 60`, `LOOP`);
            ...
            return (0, mcp_helpers_js_1.createMCPError)(error, "generating state machine template");
        }
    });
}
//# sourceMappingURL=mouse-input-tools.js.map