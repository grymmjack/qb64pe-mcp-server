...
exports.registerSessionProblemsTools = registerSessionProblemsTools;
const zod_1 = require("zod");
function registerSessionProblemsTools(server, services) {
    /**
     * Log a session problem
     */
    server.registerTool("log_session_problem", {
        ...
        inputSchema: {
            category: zod_1.z.enum(["syntax", "compatibility", "workflow", "tooling", "architecture", "other"]).optional().describe("Problem category"),
            severity: zod_1.z.enum(["critical", "high", "medium", "low"]).optional().describe("Problem severity"),
            title: zod_1.z.string().optional().describe("Brief problem title"),
            description: zod_1.z.string().optional().describe("Detailed problem description"),
            context: zod_1.z.record(zod_1.z.any()).optional().describe("Problem context - flexible object for any contextual fields"),
            problem: zod_1.z.record(zod_1.z.any()).optional().describe("Problem details - flexible object for error details, attempts, etc."),
            solution: zod_1.z.record(zod_1.z.any()).optional().describe("Solution details - flexible object for implementation, prevention, etc."),
            mcpImprovement: zod_1.z.record(zod_1.z.any()).optional().describe("MCP improvement suggestions - flexible object"),
            metrics: zod_1.z.record(zod_1.z.any()).optional().describe("Performance metrics - flexible object")
        }
    }, async (args) => {
...
Use \`get_session_problems_report\` to view all logged problems and recommendations.`,
     ...
     */
    server.registerTool("get_session_problems_report", {
        ...
        inputSchema: {
            format: zod_1.z.enum(["summary", "detailed", "markdown"]).optional().describe("Report format (default: detailed)")
        }
    }, async (args) => {
...
## Key Patterns
${report.patterns.map((p, i) => `${i + 1}. ${p}`).join('\n')}
...
Use format="detailed" or format="markdown" for complete problem details.`,
        ...
        // Detailed format
        const problemsText = report.problems.map((p, i) => `
## Problem ${i + 1}: ${p.title}
...
${problemsText}`,
     ...
     */
    server.registerTool("get_session_problems_statistics", {
...
` : '✅ All problems utilized available MCP tools appropriately.'}`,
     ...
     */
    server.registerTool("clear_session_problems", {
        ...
        inputSchema: {
            confirm: zod_1.z.boolean().describe("Must be true to confirm clearing")
        }
    }, async (args) => {
        if (!args.confirm) {
            return {
...
Old session ID has been archived.`,
     ...
     */
    server.registerTool("get_unhandled_problems", {
        ...
        inputSchema: {
            priorityFilter: zod_1.z.enum(["high-only", "all"]).optional().describe("Filter by MCP improvement priority")
        }
    }, async (args) => {
...
3. Mark as handled using \`update_problem_status\``,
    ...
    server.registerTool("update_problem_status", {
        ...
        inputSchema: {
            problemId: zod_1.z.string().describe("Problem ID to update"),
            status: zod_1.z.enum(["acknowledged", "in-progress", "handled", "wont-fix"]).describe("New status"),
            handledBy: zod_1.z.string().optional().describe("Who handled it (e.g., 'agent', 'manual-fix')"),
            notes: zod_1.z.string().optional().describe("Notes about how it was handled or why wont-fix")
        }
    }, async (args) => {
        ...
        const updated = services.sessionProblemsService.updateProblemStatus(args.problemId, args.status, args.handledBy, args.notes);
        if (!updated) {
            return {
                content: [
                    {
...
//# sourceMappingURL=session-problems-tools.js.map