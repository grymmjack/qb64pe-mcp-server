  ...
  private async setupTools(): Promise<void> {
    ...
    this.server.registerTool(
      "validate_qb64pe_compatibility",
      {
        ...
        inputSchema: {
          code: z.string().describe("QB64PE code to check for compatibility issues"),
          platform: z.enum(["windows", "macos", "linux", "all"]).optional().describe("Target platform")
        }
      },
    ...
    this.server.registerTool(
      "autocomplete_qb64pe_keywords",
      {
        ...
        inputSchema: {
          prefix: z.string().describe("The partial keyword to autocomplete"),
          maxResults: z.number().optional().describe("Maximum number of suggestions (default: 10)")
        }
      },
    ...
    this.server.registerTool(
      "search_qb64pe_keywords",
      {
        ...
        inputSchema: {
          query: z.string().describe("Search query for keywords"),
    ...
    this.server.registerTool(
      "search_qb64pe_keywords_by_wiki_category",
      {
        ...
        inputSchema: {
          ...
          query: z.string().optional().describe("Optional search query to filter keywords within the category"),
      ...
      async ({ category, query, maxResults = 50 }) => {
        try {
          ...
          if (query) {
            ...
            filteredKeywords = categoryKeywords.filter((keyword: string) => {
          ...
          const keywordDetails = limitedResults.map((keyword: string) => {
            const keywordInfo = this.keywordsService.getKeyword(keyword);
            return {
              keyword,
              category,
      ...
      async () => {
        try {
          ...
          const categoriesWithCounts = Object.entries(wikiCategories).map(([categoryName, keywords]) => ({
            ...
            keywordCount: (keywords as string[]).length,
            sampleKeywords: (keywords as string[]).slice(0, 5), // First 5 keywords as examples
          ...
          return {
            content: [{
              ...
              text: JSON.stringify({
                ...
                totalKeywords: Object.values(categoryCounts).reduce((sum: number, count: number) => sum + count, 0),
    ...
    this.server.registerTool(
      "get_process_monitoring_commands",
      {
        ...
        inputSchema: {
          processName: z.string().optional().describe("Process name to monitor (default: qb64pe)"),
          platform: z.enum(["windows", "linux", "macos", "current"]).optional().describe("Target platform (default: current)")
        }
      },
    ...
    this.server.registerTool(
      "generate_monitoring_template",
      {
        ...
        inputSchema: {
          originalCode: z.string().describe("Original QB64PE code to wrap with monitoring")
        }
      },
      async ({ originalCode }) => {
        try {
...
Remember: QB64PE installation and PATH configuration is often the first hurdle for users!`;
    ...
    this.server.registerTool(
      "port_qbasic_to_qb64pe",
      {
        ...
        inputSchema: {
          sourceCode: z.string().describe("QBasic source code to convert to QB64PE"),
          sourceDialect: z.enum([
            "qbasic", 
            "gwbasic", 
            "quickbasic", 
            "vb-dos", 
          ...
          addModernFeatures: z.boolean().optional().describe("Add modern QB64PE features like $NoPrefix, $Resize:Smooth (default: true)"),
          preserveComments: z.boolean().optional().describe("Preserve original comments (default: true)"),
          convertGraphics: z.boolean().optional().describe("Convert and enhance graphics operations (default: true)"),
          optimizePerformance: z.boolean().optional().describe("Apply performance optimizations (default: true)")
        }
      },
    ...
    this.server.registerTool(
      "analyze_qb64pe_graphics_screenshot",
      {
        ...
        inputSchema: {
          screenshotPath: z.string().describe("Path to the screenshot file to analyze (PNG, JPG, GIF)"),
          analysisType: z.enum([
            "shapes", 
            "colors", 
            "layout", 
          ...
          expectedElements: z.array(z.string()).optional().describe("List of expected visual elements to look for"),
          programCode: z.string().optional().describe("Original QB64PE code that generated the screenshot for context")
        }
      },
      async ({ screenshotPath, analysisType = "comprehensive", expectedElements, programCode }) => {
        try {
          ...
          const analysisInstructions = this.generateAnalysisInstructions(
            analysisType, 
            expectedElements, 
            codeAnalysis
          );
    ...
    this.server.registerTool(
      "capture_qb64pe_screenshot",
      {
        ...
        inputSchema: {
          outputPath: z.string().optional().describe("Path to save screenshot (auto-generated if not provided)"),
          windowTitle: z.string().optional().describe("Specific window title to capture"),
          processName: z.string().optional().describe("Process name to capture (default: qb64pe)"),
          format: z.enum(["png", "jpg", "gif"]).optional().describe("Image format (default: png)")
        }
      },
    ...
    this.server.registerTool(
      "start_screenshot_monitoring",
      {
        ...
        inputSchema: {
          checkIntervalMs: z.number().optional().describe("How often to check for processes (default: 5000ms)"),
          captureIntervalMs: z.number().optional().describe("How often to capture screenshots (default: 10000ms)")
        }
      },
    ...
    this.server.registerTool(
      "start_screenshot_watching",
      {
        ...
        inputSchema: {
          directory: z.string().optional().describe("Directory to watch (default: qb64pe-screenshots)"),
          analysisType: z.enum(["shapes", "colors", "layout", "text", "quality", "comprehensive"]).optional().describe("Type of analysis to perform (default: comprehensive)"),
          autoAnalyze: z.boolean().optional().describe("Automatically analyze new screenshots (default: true)")
        }
      },
    ...
    this.server.registerTool(
      "stop_screenshot_watching",
      {
        ...
        inputSchema: {
          directory: z.string().optional().describe("Specific directory to stop watching (empty = stop all)")
        }
      },
      async ({ directory }) => {
        try {
    ...
    this.server.registerTool(
      "enhance_qb64pe_code_for_debugging",
      {
        ...
        inputSchema: {
          ...
          config: z.object({
            enableConsole: z.boolean().optional().describe("Enable console management (default: true)"),
            enableLogging: z.boolean().optional().describe("Enable logging system (default: true)"),
            enableScreenshots: z.boolean().optional().describe("Enable screenshot system (default: true)"),
            enableFlowControl: z.boolean().optional().describe("Enable flow control for automation (default: true)"),
            enableResourceTracking: z.boolean().optional().describe("Enable resource management (default: true)"),
            timeoutSeconds: z.number().optional().describe("Timeout for automated operations (default: 30)"),
            autoExit: z.boolean().optional().describe("Enable automatic exit behavior (default: true)"),
            verboseOutput: z.boolean().optional().describe("Enable verbose debug output (default: true)")
      ...
      "inject_native_qb64pe_logging",
      {
        ...
        inputSchema: {
          ...
          config: z.object({
            ...
            outputSections: z.array(z.string()).optional().describe("Custom output sections for structured debugging")
...
- **Automation Ready**: Auto-exits to prevent hanging in automated workflows`
    ...
    this.server.registerTool(
      ...
      async ({ programName, analysisSteps, config = {} }) => {
        try {
          const template = this.loggingService.generateAdvancedDebuggingTemplate(
            programName,
            analysisSteps,
            config
          );
          ...
          const captureCommand = this.loggingService.generateOutputCaptureCommand(
            `${programName.toLowerCase().replace(/\s+/g, '_')}_debug_template.exe`
          );
...
This template is specifically designed for LLM-automated debugging workflows and systematic analysis.`
    ...
    this.server.registerTool(
      "parse_qb64pe_structured_output",
      {
        ...
        inputSchema: {
          output: z.string().describe("Raw output from QB64PE program execution")
        }
      },
      async ({ output }) => {
        try {
    ...
    this.server.registerTool(
      "generate_output_capture_commands",
      {
        ...
        inputSchema: {
          ...
          outputPath: z.string().optional().describe("Path for output file (default: analysis_output.txt)"),
          includeMonitoring: z.boolean().optional().describe("Include real-time monitoring commands (default: true)")
        }
      },
      ...
      async ({ section = "all" }) => {
        try {
          ...
          if (section !== "all") {
            ...
            const sectionRegex = sections[section as keyof typeof sections];
            if (sectionRegex) {
...
When using with \`inject_native_qb64pe_logging\`, these functions are automatically included and integrated with the native logging system.`
    ...
    this.server.registerTool(
      "generate_programming_feedback",
      {
        ...
        inputSchema: {
          screenshotPath: z.string().describe("Path to the screenshot to analyze"),
          programCode: z.string().optional().describe("Associated QB64PE program code for context")
        }
      },
    ...
    this.server.registerTool(
      "get_programming_feedback_history",
      {
        ...
        inputSchema: {
          limit: z.number().optional().describe("Maximum number of feedback entries to return (default: 10)")
        }
      },
      async ({ limit = 10 }) => {
        try {
   ...
   */
  private handleAnalysisComplete(analysisResult: any): void {
    try {
   ...
   */
  private calculateFeedbackTrends(recentFeedback: any[]): any {
  ...
  private calculateQualityTrend(feedback: any[]): string {
    const qualityValues: Record<string, number> = { poor: 1, fair: 2, good: 3, excellent: 4 };
    const recent = feedback.slice(0, 3).map(f => qualityValues[f.overallAssessment.quality as string] || 2);
    const older = feedback.slice(3, 6).map(f => qualityValues[f.overallAssessment.quality as string] || 2);
  ...
  private calculateCompletenessTrend(feedback: any[]): string {
  ...
  private calculateSuggestionsTrend(feedback: any[]): string {
  ...
  private determineOverallTrend(quality: string, completeness: string, suggestions: string): string {
   ...
   */
  private generateStatisticsRecommendations(statistics: any, trends: any): string[] {
    ...
    if (Object.values(statistics.qualityDistribution || {}).some((count: any) => count > statistics.total * 0.5)) {
   ...
   */
  private getAutomationRecommendations(screenshotStatus: any, watcherStatus: any): string[] {
    const recommendations: string[] = [];
  ...
  private extractScreenSize(code: string): string {
  ...
  private extractTextElements(code: string): string[] {
  ...
  private generateLLMGuidance(analysisType: string, codeAnalysis?: any): any {
    return {
  ...
  private generateBasicShapesTemplate(): string {
...
System 0`;
  ...
  private generateColorPaletteTemplate(): string {
...
System 0`;
  ...
  private generateTextRenderingTemplate(): string {
...
System 0`;
  ...
  private generateLayoutGridTemplate(): string {
...
System 0`;
  ...
  private generateCustomTemplate(specs: any): string {
...
Focus specifically on QB64PE debugging techniques and tools.`
    ...
    this.server.registerPrompt(
      "monitor-qb64pe-execution",
      {
        ...
        argsSchema: {
          sourceCode: z.string().describe("QB64PE source code to analyze"),
          expectedBehavior: z.string().optional().describe("Expected program behavior"),
          platform: z.enum(["windows", "macos", "linux"]).optional().describe("Target platform")
        }
      },
    ...
    this.server.registerPrompt(
      "debug-qb64pe-comprehensive",
      {
        ...
        argsSchema: {
          ...
          problemDescription: z.string().optional().describe("Description of the problem being experienced"),
          executionContext: z.enum(["automated", "interactive", "testing"]).optional().describe("How the program will be executed"),
          platform: z.enum(["windows", "macos", "linux"]).optional().describe("Target platform")
        }
      },
    ...
    this.server.registerPrompt(
      "port-qbasic-to-qb64pe",
      {
        ...
        argsSchema: {
          ...
          sourceDialect: z.enum(["qbasic", "gwbasic", "quickbasic", "vb-dos"]).optional().describe("Source BASIC dialect"),
          preserveOriginal: z.string().optional().describe("Whether to preserve original formatting and comments (true/false)")
        }
      },
...
- Suggest modern QB64PE features that could enhance the program`
   ...
   */
  private getCompatibilityRecommendations(result: any, dialect: string): string[] {
    const recommendations: string[] = [];
  ...
  private generateSessionRecommendations(session: any): string[] {
    ...
    if (session.issues.length === 0) {
      ...
      const criticalIssues = session.issues.filter((i: any) => i.severity === 'critical');
      const highIssues = session.issues.filter((i: any) => i.severity === 'high');
  ...
  private getSessionRecommendedActions(session: any): string[] {
    ...
    if (session.executionMode === 'graphics' && !session.solutions.some((s: any) => s.strategy === 'code_injection')) {
   ...
   */
  private getExecutionModeDistribution(sessions: any[]): Record<string, number> {
