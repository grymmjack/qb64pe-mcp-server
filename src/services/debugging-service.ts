...
export class QB64PEDebuggingService {
  ...
  enhanceCodeForDebugging(
    ...
    if (mergedConfig.enableConsole) {
      const consoleEnhancement = this.injectConsoleManagement(
        enhancedCode,
        mergedConfig,
      );
    ...
    if (mergedConfig.enableFlowControl) {
      const flowEnhancement = this.injectFlowControl(
        enhancedCode,
        mergedConfig,
      );
    ...
    if (
      ...
      const graphicsEnhancement =
        this.injectGraphicsContextManagement(enhancedCode);
    ...
    if (mergedConfig.enableLogging) {
      const loggingEnhancement = this.injectLoggingSystem(
        enhancedCode,
        mergedConfig,
      );
    ...
    if (
      mergedConfig.enableScreenshots &&
      (session.executionMode === "graphics" ||
        session.executionMode === "mixed")
    ) {
  ...
  private injectConsoleManagement(
    ...
    if (!code.includes("$CONSOLE")) {
      code = `$CONSOLE:ONLY\n${code}`;
      modifications.push(
        "Added $CONSOLE:ONLY directive for shell redirection compatibility",
      );
      ...
      modifications.push(
        "Updated $CONSOLE to $CONSOLE:ONLY for shell redirection compatibility",
      );
    }
...
`;
    ...
    code = code.replace(
      /PRINT\s+"Press any key[^"]*"\s*[\r\n]+\s*SLEEP/gi,
      'CALL DebugPause("Press any key to continue...")',
    );
    modifications.push(
      "Replaced blocking SLEEP statements with conditional pauses",
    );
...
`;
    ...
    modifications.push(
      "Added automated screenshot system with visual debugging markers",
    );
  ...
  private detectExecutionMode(
    sourceCode: string,
  ): "console" | "graphics" | "mixed" {
    const hasConsole = /\$CONSOLE/i.test(sourceCode);
    const hasGraphics =
      /\b(SCREEN|CIRCLE|LINE|PSET|POINT|PAINT|PUT|GET|_NEWIMAGE|_PUTIMAGE|_LOADIMAGE)\b/i.test(
        sourceCode,
      );
  ...
  private analyzeCodeForIssues(sourceCode: string): DebuggingIssue[] {
    ...
    if (
      ...
      issues.push({
        ...
        severity: "high",
        description:
          "Program may exit immediately without showing console output",
        symptoms: [
    ...
    if (blockingSleepPattern.test(sourceCode)) {
      issues.push({
        ...
        severity: "high",
        description:
          'Program contains blocking "Press any key" prompts that prevent automation',
        symptoms: [
    ...
    switch (issue.type) {
      case "console_visibility":
        solutions.push({
          ...
          priority: 1,
          description:
            "Inject console management system with $CONSOLE directive and Windows console activation",
          implementation: {
            codeChanges: [
          ...
          rationale:
            "QB64PE programs need explicit console activation to ensure output visibility",
        });
        break;
      ...
      case "flow_control":
        solutions.push({
          ...
          priority: 1,
          description:
            "Replace blocking user input with conditional automation-friendly pauses",
          implementation: {
            codeChanges: [
          ...
          rationale:
            "Automated systems need timeout mechanisms to prevent indefinite hanging",
        });
        break;
      ...
      case "file_handle":
        solutions.push({
          ...
          priority: 2,
          description:
            "Implement resource management system to track and cleanup file handles",
          implementation: {
            codeChanges: [
          ...
          rationale:
            "Proper resource management prevents file locking and compilation errors",
        });
        break;
      ...
      case "graphics_context":
        solutions.push({
          ...
          priority: 2,
          description:
            "Add graphics context management with automatic cleanup and bounds checking",
          implementation: {
            codeChanges: [
          ...
          rationale:
            "Graphics programs need careful resource management to prevent memory leaks and crashes",
        });
        break;
      ...
      case "process_management":
        solutions.push({
          ...
          rationale:
            "Controlled program termination allows proper observation and cleanup",
        });
        break;
    }
...
`;
  ...
  getActiveSessions(): DebuggingSession[] {
    return Array.from(this.debugSessions.values()).filter(
