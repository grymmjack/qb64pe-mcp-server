...
export class KeywordsService {
   ...
   */
  public searchMCPDocs(
    keyword: string
  ): Array<{ file: string; context: string; line: number }> {
    const results: Array<{ file: string; context: string; line: number }> = [];
  ...
  private parsePlatformAvailability(
    ...
    if (
      (lower.includes("windows") && lower.includes("linux")) ||
  ...
  private loadWikiCategoriesData(): void {
    try {
      const wikiCategoriesPath = path.join(
        __dirname,
        "../../docs/resources/QB64PE Keywords by Category.json"
      );
  ...
  private loadKeywordsData(): void {
    try {
      ...
      const originalPath = path.join(
        __dirname,
        "../../docs/resources/QB64PE_Keywords.json"
      );
      ...
      if (fs.existsSync(enhancedPath)) {
        ...
        const enhancedKeywordCount = Object.keys(
  ...
  private categorizeKeyword(
    ...
    if (
      (lowerDesc.includes("returns") && !lowerDesc.includes("statement")) ||
      name.endsWith("$") ||
  ...
  private extractAvailability(
    ...
    if (
      (lower.includes("not") || lower.includes("not available")) &&
  ...
  private generateExample(
    ...
    const exampleMap: Record<string, string> = {
      ...
      CIRCLE:
        "CIRCLE (100, 100), 50 ' Circle at (100,100) with radius 50\nCIRCLE (200, 200), 30, _RGB32(255, 0, 0) ' Red circle",
  ...
  public getKeywordsByType(type: KeywordInfo["type"]): KeywordInfo[] {
    return Object.values(this.keywordsData.keywords).filter(
  ...
  public getDeprecatedKeywords(): KeywordInfo[] {
    return Object.values(this.keywordsData.keywords).filter(
      (keyword) => keyword.deprecated === true
    );
  }
  ...
  public getKeywordsByTag(tag: string): KeywordInfo[] {
    return Object.values(this.keywordsData.keywords).filter((keyword) =>
      keyword.tags?.includes(tag)
    );
  }

  public getQB64PESpecificKeywords(): KeywordInfo[] {
    return Object.values(this.keywordsData.keywords).filter(
      (keyword) => keyword.version === "QB64PE" || keyword.version === "QB64"
    );
  }

  public getLegacyKeywords(): KeywordInfo[] {
    return Object.values(this.keywordsData.keywords).filter(
      (keyword) => keyword.version === "QBasic" || keyword.type === "legacy"
    );
  }
   ...
   */
  public updatePlatformAvailability(
    keywordName: string,
    windows: boolean,
    linux: boolean,
    macos: boolean
    ...
    let availability: KeywordInfo["availability"];
   ...
   */
  public batchUpdatePlatformAvailability(
    platformData: Map<
      string,
    ...
    for (const [keywordName, data] of platformData.entries()) {
      const success = this.updatePlatformAvailability(
        keywordName,
        data.windows,
        data.linux,
        data.macos
