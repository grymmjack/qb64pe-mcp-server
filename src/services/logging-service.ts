...
export interface LoggingConfiguration {
  ...
  enableEchoOutput: boolean; // Simplified console output without _DEST management
...
END`;
   ...
   */
  public generateOutputCaptureCommands(
    programPath: string,
    outputPath: string = "program_output.txt",
   ...
   */
  public generateFileMonitoringCommands(
    logFile: string,
  ): Record<string, string> {
    return {
...
`;
    ...
    if (endIndex !== -1) {
      return (
        code.substring(0, endIndex) + autoExit + "\n" + code.substring(endIndex)
      );
    }
