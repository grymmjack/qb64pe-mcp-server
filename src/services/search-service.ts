...
export class CompatibilitySearchService {
    ...
    const compatibilityDocs = [
      {
        ...
        content:
          "QB64PE doesn't support AS TYPE syntax for function return types. Use type sigils instead: % INTEGER & LONG ! SINGLE # DOUBLE $ STRING",
      ...
      },
      {
        ...
        content:
          "$CONSOLE:OFF is not valid syntax. Use $CONSOLE for both console and graphics windows, or $CONSOLE:ONLY for console mode only",
      ...
      },
      {
        ...
        content:
          "Chained IF-THEN statements with colons cause parsing errors. Split IF statements onto separate lines to avoid THEN without IF errors",
      ...
      },
      {
        ...
        content:
          "Multiple arrays with dimensions cannot be declared on one line. DIM array declarations should be separate when using dimensions",
      ...
      },
      {
        ...
        content:
          "Combining declarations and assignments with colons can cause parsing issues. Separate variable declarations from assignments onto different lines",
      ...
      },
      {
        ...
        content:
          "_WORD$ and _TRIM$ functions don't exist in QB64PE. Use built-in string functions like INSTR, MID$, LEFT$, RIGHT$ for string parsing instead",
      ...
      },
      {
        ...
        content:
          "Legacy BASIC keywords not supported: DEF FN, TRON, TROFF, SETMEM, SIGNAL, ERDEV, ERDEV$, FILEATTR, FRE, IOCTL, IOCTL$. Use modern QB64PE alternatives",
      ...
      },
      {
        ...
        content:
          "Device access keywords not supported: ON PEN, PEN, ON PLAY(n), PLAY(n) ON/OFF/STOP, ON UEVENT, UEVENT. Use modern QB64PE input/output methods like _MOUSEINPUT, _MOUSEBUTTON",
      ...
      },
      {
        ...
        content:
          "Some functions are Windows-only: _ACCEPTFILEDROP, _TOTALDROPPEDFILES, _DROPPEDFILE, _FINISHDROP, _SCREENPRINT, _SCREENCLICK, _WINDOWHANDLE, _CAPSLOCK, _NUMLOCK, _SCROLLLOCK",
      ...
      },
      {
        ...
        content:
          "Console functions may not be available on Linux/macOS: _CONSOLETITLE, _CONSOLECURSOR, _CONSOLEFONT, _CONSOLEINPUT, _CINP. Use standard INPUT/PRINT or check platform compatibility",
      ...
      },
      {
        ...
        content:
          "Check platform with _OS$ before using platform-specific features. Provide alternatives for Linux/macOS when using Windows-only functions",
  ...
  async searchKeywords(
    ...
    const keywordResults = this.keywordsService.searchKeywords(
      query,
      maxResults,
    );
    ...
    const keywordDocs = searchResults.filter(
      (result) => result.document.metadata?.isKeyword === true,
    );
    ...
    keywordResults.forEach((kResult) => {
      ...
      if (!combined.has(key)) {
        ...
        const doc = keywordDocs.find(
          (d) => d.document.metadata?.keywordName === kResult.keyword,
        );
  ...
  async searchKeywordsByCategory(category: string): Promise<SearchResult[]> {
    ...
    keywords.forEach((keyword) => {
      const doc = this.index.documents.find(
        (d) => d.metadata?.keywordName === keyword.name,
      );
