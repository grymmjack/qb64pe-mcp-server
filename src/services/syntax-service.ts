...
export class QB64PESyntaxService {
  ...
  private checkLoadImageCalls(
    ...
    const loadImageConcatPattern =
      /\b_LOADIMAGE\s*\(\s*([^,)]+(?:\+[^,)]+)+)\s*[,)]/gi;
    ...
    lines.forEach((line, index) => {
      ...
      if (loadImageDirPattern.test(line)) {
        compatibilityIssues.push({
          ...
          pattern: "_LOADIMAGE",
          message:
            "_LOADIMAGE path ends with a directory separator — no filename is included. This causes a std::bad_alloc crash (exit code 134) with no QB64 line info.",
          severity: "error",
          category: "loadimage_empty_path",
          suggestion:
            "Ensure the path argument includes a filename, not just a directory. Example: _LOADIMAGE(dir$ + filename$) where filename$ is a non-empty string.",
          examples: {
            incorrect: '_LOADIMAGE("ASSETS/THEMES/DEFAULT/IMAGES/EDITBAR/")',
            correct:
              '_LOADIMAGE("ASSETS/THEMES/DEFAULT/IMAGES/EDITBAR/undo.png")',
          },
        });
        return;
      }
      ...
      if (concatMatch) {
        ...
        const hasStructField = /\b[A-Z_][A-Z0-9_]*\.[A-Z_][A-Z0-9_]*\$?/i.test(
          expr,
        );
        compatibilityIssues.push({
          line: lineNum,
          ...
          pattern: "_LOADIMAGE",
          message:
            "_LOADIMAGE path is a concatenated expression" +
            (hasStructField
      ...
      if (loadImageVarPattern.test(line)) {
        compatibilityIssues.push({
          ...
          pattern: "_LOADIMAGE",
          message:
            "_LOADIMAGE is called with a variable or expression argument that could be an empty string. " +
          ...
          category: "loadimage_empty_path",
          suggestion:
            "Validate the path before calling _LOADIMAGE: IF LEN(_TRIM$(path$)) > 0 THEN handle% = _LOADIMAGE(path$). " +
          ...
          examples: {
            incorrect:
              "handle% = _LOADIMAGE(imagePath$)  ' imagePath$ may be empty",
            correct:
              "IF LEN(_TRIM$(imagePath$)) > 0 THEN\n    handle% = _LOADIMAGE(imagePath$)\nEND IF",
          },
        });
      }
    });
