...
export class ValidationService {
  ...
  validateCode(
    ...
    const {
      maxLength = 100000, // 100KB default max
      minLength = 0,
      allowEmpty = false,
      checkEncoding = true,
    } = options;
    ...
    if (code.length < minLength) {
      errors.push(
        `Code is too short (${code.length} chars, minimum ${minLength})`,
      );
    }
    ...
    if (code.length > maxLength) {
      errors.push(
        `Code is too long (${code.length} chars, maximum ${maxLength})`,
      );
    }
    ...
    if (code.length > maxLength * 0.8) {
      warnings.push(
        `Code is very large (${code.length} chars) - consider splitting into modules`,
      );
    }
    ...
    if (checkEncoding) {
      try {
        ...
        if (nonPrintable.test(code)) {
          warnings.push(
            "Code contains non-printable characters - may cause issues",
          );
        }
      } catch (err) {
  ...
  validatePath(
    ...
    const detectedPlatform =
      platform === "auto" ? this.detectPathPlatform(trimmedPath) : platform;
    ...
    if (trimmedPath.includes("/") && trimmedPath.includes("\\")) {
      warnings.push(
        "Path contains mixed separators (/ and \\) - may cause issues",
      );
    }
    ...
    if (allowedExtensions.length > 0) {
      const hasValidExtension = allowedExtensions.some((ext) =>
        trimmedPath.toLowerCase().endsWith(ext.toLowerCase()),
      );
      if (!hasValidExtension) {
        errors.push(
          `Path must end with one of: ${allowedExtensions.join(", ")}`,
        );
      }
    }
    ...
    if (trimmedPath.length > maxPathLength * 0.8) {
      warnings.push(
        `Path is very long (${trimmedPath.length} chars) - may exceed system limits`,
      );
    }
  ...
  validateRequiredString(
    ...
    if (trimmed.length < minLength) {
      errors.push(
        `${paramName} is too short (${trimmed.length} chars, minimum ${minLength})`,
      );
    }

    if (trimmed.length > maxLength) {
      errors.push(
        `${paramName} is too long (${trimmed.length} chars, maximum ${maxLength})`,
      );
    }
    ...
    if (itemValidator) {
      value.forEach((item, index) => {
        ...
        if (!itemResult.isValid) {
          errors.push(
            `${paramName}[${index}]: ${itemResult.errors.join(", ")}`,
          );
        }
        warnings.push(
          ...itemResult.warnings.map((w) => `${paramName}[${index}]: ${w}`),
        );
      });
    }
  ...
  validateEnum(
    ...
    if (!allowedValues.includes(value)) {
      errors.push(
        `${paramName} must be one of: ${allowedValues.join(", ")} (received: ${value})`,
      );
    }
  ...
  private detectPathPlatform(path: string): "windows" | "unix" {
  ...
  private isAbsolutePath(path: string, platform: "windows" | "unix"): boolean {
    if (platform === "windows") {
