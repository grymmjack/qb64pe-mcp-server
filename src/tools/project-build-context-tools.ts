...
import { z } from "zod";
import { ServiceContainer } from "../utils/tool-types";
import {
  createMCPResponse,
  createMCPError,
...
export function registerProjectBuildContextTools(
  server: any,
  ...
  server.registerTool(
    "get_project_build_context",
    {
      ...
      description:
        "🔍 Retrieve stored build configuration for a QB64PE project.\n\n" +
    ...
    async ({ sourceFilePath }: { sourceFilePath: string }) => {
      try {
        const context =
          await services.compilerService.buildContextService.loadContext(
            sourceFilePath,
          );

        if (!context) {
          return createMCPTextResponse(
            `📋 **No Build Context Found**\n\n` +
        ...
        const stats =
          await services.compilerService.buildContextService.getProjectStatistics(
            sourceFilePath,
          );
        ...
        return createMCPError(error, "getting project build context");
      }
    },
  );
  ...
  server.registerTool(
    "list_project_build_contexts",
    {
      ...
      description:
        "📚 List all QB64PE projects that have stored build contexts.\n\n" +
    ...
    async () => {
      try {
        const projects =
          await services.compilerService.buildContextService.listProjects();

        if (projects.length === 0) {
          return createMCPTextResponse(
            `📚 **No Project Build Contexts**\n\n` +
        ...
        return createMCPError(error, "listing project build contexts");
      }
    },
  );
  ...
  server.registerTool(
    "clear_project_build_context",
    {
      ...
      description:
        "🗑️ Clear stored build context for a specific project.\n\n" +
    ...
    async ({ sourceFilePath }: { sourceFilePath: string }) => {
      try {
        const success =
          await services.compilerService.buildContextService.clearContext(
            sourceFilePath,
          );

        if (success) {
          return createMCPTextResponse(
            `✅ **Build Context Cleared**\n\n` +
              `Successfully removed build context for:\n${sourceFilePath}\n\n` +
              `A new context will be created on the next build.`,
          );
        } else {
          return createMCPTextResponse(
            `⚠️ **No Context to Clear**\n\n` +
              `No build context found for:\n${sourceFilePath}`,
          );
        }
      } catch (error) {
        return createMCPError(error, "clearing project build context");
      }
    },
  );
  ...
  server.registerTool(
    "get_build_context_statistics",
    {
      ...
      description:
        "📊 Get detailed build statistics for a project.\n\n" +
    ...
    async ({ sourceFilePath }: { sourceFilePath: string }) => {
      try {
        ...
        if (stats.mostUsedFlags.length > 0) {
          ...
          stats.mostUsedFlags.forEach((flag: string, index: number) => {
        ...
        return createMCPError(error, "getting build statistics");
      }
    },
  );
}
