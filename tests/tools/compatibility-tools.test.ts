...
describe("Compatibility Tools", () => {
  ...
  beforeEach(() => {
    ...
    mockCompatibilityService = {
      ...
      validateKeyboardBufferSafety: jest.fn(),
    };
  ...
  describe("registerCompatibilityTools", () => {
    ...
    it("should register exactly 6 compatibility tools", () => {
      registerCompatibilityTools(mockServer, services);
    ...
    it("should register all expected tool names", () => {
      ...
      expect(toolNames).toContain("validate_qb64pe_compatibility_file");
      ...
      expect(toolNames).toContain("validate_keyboard_buffer_safety_file");
    });
  });
  ...
  describe("validate_qb64pe_compatibility", () => {
    it("should validate compatibility successfully", async () => {
      ...
      expect(
        mockCompatibilityService.validateCompatibility,
      ).toHaveBeenCalledWith('PRINT "test"');
      expect(
        mockCompatibilityService.getPlatformCompatibility,
    ...
    it("should validate compatibility from sourceFilePath", async () => {
      const fs = require("fs");
      const path = require("path");
      const testFile = path.join(process.cwd(), "compatibility-test.bas");
  ...
  describe("search_qb64pe_compatibility", () => {
    it("should search compatibility knowledge successfully", async () => {
      ...
      mockCompatibilityService.searchCompatibility.mockResolvedValue(
        mockResults,
      );

      await handler!({ query: "function types" });

      expect(mockCompatibilityService.searchCompatibility).toHaveBeenCalledWith(
        "function types",
      );
    ...
    it("should handle search errors", async () => {
      ...
      expect(createMCPError).toHaveBeenCalledWith(
        error,
        "searching compatibility knowledge",
      );
    });
  });
  ...
  describe("get_qb64pe_best_practices", () => {
    it("should get best practices for all topics", async () => {
      ...
      mockCompatibilityService.getBestPractices.mockResolvedValue(
        mockPractices,
      );
      mockCompatibilityService.getDebuggingGuidance.mockResolvedValue(
        mockDebuggingHelp,
      );
    ...
    it("should skip debugging section for non-debugging topics", async () => {
      ...
      expect(mockCompatibilityService.getBestPractices).toHaveBeenCalled();
      expect(
        mockCompatibilityService.getDebuggingGuidance,
  ...
  describe("validate_keyboard_buffer_safety", () => {
    it("should validate keyboard buffer safety successfully", async () => {
      ...
      mockCompatibilityService.validateKeyboardBufferSafety = jest
        .fn()
      ...
      expect(
        mockCompatibilityService.validateKeyboardBufferSafety,
      ).toHaveBeenCalledWith("IF _KEYDOWN(27) THEN END");
      expect(createMCPTextResponse).toHaveBeenCalled();
    });
    ...
    it("should handle code with no issues", async () => {
      ...
      const mockResult = {
        ...
        issues: [],
        suggestions: [
          "Good practice: Your code includes keyboard buffer drains",
        ],
        bestPractices: ["Use DO WHILE _KEYHIT: LOOP"],
        summary: {
          totalIssues: 0,
    ...
    it("should handle validation errors", async () => {
      ...
      expect(createMCPError).toHaveBeenCalledWith(
        error,
        "validating keyboard buffer safety",
      );
    });
  });
});
