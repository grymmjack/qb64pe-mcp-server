...
describe("Porting Tools", () => {
  ...
  describe("registerPortingTools", () => {
    ...
    it("should register port_qbasic_to_qb64pe tool", () => {
      ...
      expect(toolNames).toContain("port_qbasic_file_to_qb64pe");
    });
    ...
    it("should register get_porting_dialect_info tool", () => {
      ...
      expect(toolNames).toContain("analyze_qbasic_file_compatibility");
    });
  });
  ...
  describe("Tool Handler Execution", () => {
    ...
    it("should execute port_qbasic_file_to_qb64pe handler", async () => {
      const fs = require("fs");
      const path = require("path");
      const testFile = path.join(process.cwd(), "legacy-port.bas");
      ...
      try {
        registerPortingTools(mockServer, services);
        const call = (mockServer.registerTool as jest.Mock).mock.calls.find(
          (c) => c[0] === "port_qbasic_file_to_qb64pe",
        );

        if (call) {
          const handler = call[2];
          const result = await handler({ sourceFilePath: testFile });
          expect(result).toBeDefined();
        }
      } finally {
    ...
    it("should execute analyze_qbasic_file_compatibility handler", async () => {
      const fs = require("fs");
      const path = require("path");
      const testFile = path.join(process.cwd(), "legacy-analyze.bas");

      fs.writeFileSync(testFile, "SCREEN 12\nCIRCLE (100, 100), 50", "utf-8");

      try {
        registerPortingTools(mockServer, services);
        const call = (mockServer.registerTool as jest.Mock).mock.calls.find(
          (c) => c[0] === "analyze_qbasic_file_compatibility",
        );
    ...
    it("should handle errors in port_qbasic_to_qb64pe gracefully", async () => {
      ...
      const errorServices = {
        portingService: {
          portQBasicToQB64PE: jest
            .fn()
            .mockRejectedValue(new Error("Porting failed")),
        },
      } as any;
    ...
    it("should handle errors in get_porting_dialect_info gracefully", async () => {
      ...
      const errorServices = {
        portingService: {
          getSupportedDialects: jest.fn().mockImplementation(() => {
            throw new Error("Failed");
          }),
          getDialectRules: jest.fn().mockImplementation(() => {
            throw new Error("Failed");
          }),
        },
      } as any;
